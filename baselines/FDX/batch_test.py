...
# Scalability data base (NEW)
WIN_SCAL_BASE_ABS = r"D:\CodeWork\python\data_dependency\code-for-paper\datasets\scalability"
SCAL_BASE_REL = os.path.relpath(WIN_SCAL_BASE_ABS, start=os.getcwd())

def scal_path(*parts: str) -> str:
    """Build a path under the scalability base, using a path relative to CWD."""
    return os.path.normpath(os.path.join(SCAL_BASE_REL, *parts))
...
# ---------------------------
# Scalability batch: iterate all *.csv in scalability base
# ---------------------------
def run_jobs_scalability(out_root: str):
    in_dir = scal_path()
    ...
    print(f"[INFO] {len(all_files)} scalability datasets to process:")
    for p in all_files:
        print("   -", os.path.basename(p))
    ...
    summaries, errors = [], []
    summary_path = os.path.join(out_root, "summary_scalability_jobs.jsonl")
    with open(summary_path, "w", encoding="utf-8") as sf:
        for path in all_files:
            sname = infer_schema_name(path)  # None -> auto
            print(f"\n=== RUN JOB (scalability) ===\npath={path}\nschema={sname or 'auto'}")
...
if __name__ == "__main__":
    ...
    # Run noise datasets
    run_jobs_noise_only(os.path.join(OUT, "noise"))

    # To run scalability datasets, uncomment the line below:
    # run_jobs_scalability(os.path.join(OUT, "scalability"))
